...
import { TaskFilterValue, TaskSort, TaskStatus } from '../../models/task.model';
...
export class TaskFiltersComponent {
  ...
  readonly form = inject(FormBuilder).nonNullable.group({
    ...
    sort: new FormControl<TaskSort>('created-desc', { nonNullable: true }),
  });
  constructor() {
  ...
  get hasActiveFilters(): boolean {
    ...
    return value.query.trim().length > 0 || value.status !== 'all' || value.sort !== 'created-desc';
  }
  /** Restore the assignment's default search, status, and ordering values. */
  clear(): void {
    this.form.setValue({ query: '', status: 'all', sort: 'created-desc' });
  }
}
